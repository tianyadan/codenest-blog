...
export const scanContent = (rootDir) => {
  ...
  for (const lang of SUPPORTED_LANGS) {
    ...
    for (const filePath of articleFiles) {
      ...
      const category = ['learning', 'work', 'diary', 'syntax', 'security'].includes(rawCategory) ? rawCategory : 'learning';
    ...
    for (const filePath of promptFiles) {
      ...
      const { data, content } = parseFrontmatter(raw);
      const slug = toStringValue(data.slug, fileSlug(filePath));
      const rawCategory = toStringValue(data.category, 'frontend');
      const category = PROMPT_CATEGORIES.includes(rawCategory) ? rawCategory : 'frontend';

      prompts.push({
        id: toStringValue(data.id, `prompt-${lang}-${slug}`),
        lang,
        slug,
        title: toStringValue(data.title, slug),
        summary: toStringValue(data.summary),
...
import type { ArticleCategory, Difficulty, Language, PromptCategory, QuestionBank } from '../../types/content';
